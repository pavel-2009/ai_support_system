...
class TestLLMTasks:
    ...
    def test_process_llm_task_retries_on_exception(self):
        ...
        with patch("app.celery.tasks.llm_tasks.asyncio.run", side_effect=_failing_run):
    ...
    def test_process_llm_task_returns_ok(self):
        def _successful_run(coro):
            coro.close()
            return None

        with patch("app.celery.tasks.llm_tasks.asyncio.run", side_effect=_successful_run) as run_mock:
            from app.celery.tasks.llm_tasks import process_llm_task
