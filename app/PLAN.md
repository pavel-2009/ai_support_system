...
- Исправлен регресс логина после рефакторинга: `POST /api/auth/login` снова принимает JSON (и остаётся совместим с OAuth2 form-data).
- Добавлены e2e-тесты пользовательского цикла и полного conversation flow без AI на SQLite test DB.
...
- [x] Исправлен регресс в авторизации JSON login после рефакторинга.
- [x] Добавлены e2e-тесты для user-profile цикла и полного conversation flow (без AI) с SQLite.
